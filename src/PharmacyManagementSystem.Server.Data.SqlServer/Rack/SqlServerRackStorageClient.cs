...
public class SqlServerRackStorageClient(ILogger<SqlServerRackStorageClient> logger, ISqlServerDbClient dbClient) : IRackStorageClient
{
    private readonly ILogger<SqlServerRackStorageClient> _logger = logger;
    ...
    public async Task<Common.Rack.Rack?> AddAsync(Common.Rack.Rack? rack, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Adding rack with label: {Label}.", rack.Label);
        ...
        _logger.LogDebug("StorageClient: Added rack with label: {Label}.", rack.Label);
