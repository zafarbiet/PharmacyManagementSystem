...
public class SqlServerDebtPaymentStorageClient(ILogger<SqlServerDebtPaymentStorageClient> logger, ISqlServerDbClient dbClient) : IDebtPaymentStorageClient
{
    private readonly ILogger<SqlServerDebtPaymentStorageClient> _logger = logger;
    ...
    public async Task<IReadOnlyCollection<Common.DebtPayment.DebtPayment>?> GetByFilterCriteriaAsync(DebtPaymentFilter filter, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Getting debt payments by filter criteria.");
        ...
        _logger.LogDebug("StorageClient: Retrieved {Count} debt payments.", list.Count);
    ...
    public async Task<Common.DebtPayment.DebtPayment?> GetByIdAsync(string id, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Getting debt payment by id: {Id}.", id);
        ...
        _logger.LogDebug("StorageClient: Retrieved debt payment with id: {Id}.", id);
    ...
    public async Task<Common.DebtPayment.DebtPayment?> AddAsync(Common.DebtPayment.DebtPayment? debtPayment, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Adding debt payment.");
        ...
        _logger.LogDebug("StorageClient: Added debt payment.");
    ...
    public async Task<Common.DebtPayment.DebtPayment?> UpdateAsync(Common.DebtPayment.DebtPayment? debtPayment, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Updating debt payment with id: {Id}.", debtPayment.Id);
        ...
        _logger.LogDebug("StorageClient: Updated debt payment with id: {Id}.", debtPayment.Id);
    ...
    public async Task RemoveAsync(Guid id, string updatedBy, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Removing debt payment with id: {Id}.", id);

        var sql = await DebtPaymentDatabaseCommandText.GetSoftDeleteSql(id, updatedBy).ConfigureAwait(false);
        await _dbClient.ExecuteAsync(sql, cancellationToken).ConfigureAwait(false);

        _logger.LogDebug("StorageClient: Removed debt payment with id: {Id}.", id);
    }
}
