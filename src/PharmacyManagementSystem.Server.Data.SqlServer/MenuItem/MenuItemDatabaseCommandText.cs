...
namespace PharmacyManagementSystem.Server.Data.SqlServer.MenuItem;
...
public static class MenuItemDatabaseCommandText
{
    ...
    public static Task<DatabaseSqlWithParameters> GetSelectSql(MenuItemFilter filter)
    {
        ...
        sql += " AND IsActive = 1 ORDER BY OrderIndex";
    ...
    public static Task<DatabaseSqlWithParameters> GetSelectByIdSql(string id)
    {
        ...
        {
            SqlStatement = $"SELECT {SelectColumns} FROM PMS.MenuItems WHERE Id = @Id AND IsActive = 1",
    ...
    public static Task<DatabaseSqlWithParameters> GetForUserSql(string username)
    {
        ...
        return Task.FromResult(new DatabaseSqlWithParameters
        {
            ...
            WHERE mi.IsActive = 1
              AND (
                NOT EXISTS (SELECT 1 FROM PMS.RoleMenuItems WHERE IsActive = 1)
                OR EXISTS (
                  SELECT 1 FROM PMS.RoleMenuItems rmi
                  INNER JOIN PMS.UserRoles ur ON ur.RoleId = rmi.RoleId AND ur.IsActive = 1
                  INNER JOIN PMS.Users au ON au.Id = ur.UserId AND au.IsActive = 1
                  WHERE rmi.MenuItemId = mi.Id AND rmi.IsActive = 1 AND au.Username = @Username
                )
              )
    ...
    public static Task<DatabaseSqlWithParameters> GetInsertSql(Common.MenuItem.MenuItem menuItem)
    {
        ...
        return Task.FromResult(new DatabaseSqlWithParameters
        {
            SqlStatement = @"INSERT INTO PMS.MenuItems (Id, [Key], Label, Icon, ParentKey, OrderIndex, UpdatedAt, UpdatedBy, IsActive)
                             OUTPUT INSERTED.*
                             VALUES (NEWID(), @Key, @Label, @Icon, @ParentKey, @OrderIndex, @UpdatedAt, @UpdatedBy, 1)",
            Parameters = parameters
        });
    }
    ...
    public static Task<DatabaseSqlWithParameters> GetSoftDeleteSql(Guid id, string updatedBy)
    {
        ...
        return Task.FromResult(new DatabaseSqlWithParameters
        {
            SqlStatement = @"UPDATE PMS.MenuItems SET IsActive = 0, UpdatedAt = @UpdatedAt, UpdatedBy = @UpdatedBy
                             OUTPUT INSERTED.*
                             WHERE Id = @Id",
            Parameters = parameters
        });
