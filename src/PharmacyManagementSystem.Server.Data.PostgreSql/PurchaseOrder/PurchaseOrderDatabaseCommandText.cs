...
public static class PurchaseOrderDatabaseCommandText
{
    ...
    public static Task<DatabaseSqlWithParameters> GetSelectByIdSql(string id)
    {
        ...
        return Task.FromResult(new DatabaseSqlWithParameters
        {
            SqlStatement = $"SELECT {SelectColumns} FROM PMS.PurchaseOrders WHERE Id = @Id AND IsActive = true",
            Parameters = parameters
        });
    }
    ...
    public static Task<DatabaseSqlWithParameters> GetInsertSql(Common.PurchaseOrder.PurchaseOrder purchaseOrder)
    {
        ...
        return Task.FromResult(new DatabaseSqlWithParameters
        {
            SqlStatement = @"INSERT INTO PMS.PurchaseOrders
                             ...
                             RETURNING *
                             VALUES
                             (gen_random_uuid(), @VendorId, @OrderDate, @Status, @Notes, @TotalAmount, @QuotationId,
                              @PoNumber, @ApprovedBy, @ApprovedAt, @ParentPurchaseOrderId, @BranchId,
                              @UpdatedAt, @UpdatedBy, @IsActive)",
            Parameters = parameters
        });
