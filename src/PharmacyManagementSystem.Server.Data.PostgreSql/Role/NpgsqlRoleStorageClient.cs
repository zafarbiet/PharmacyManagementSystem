...
namespace PharmacyManagementSystem.Server.Data.PostgreSql.Role;

public class NpgsqlRoleStorageClient(ILogger<NpgsqlRoleStorageClient> logger, INpgsqlDbClient dbClient) : IRoleStorageClient
{
    private readonly ILogger<NpgsqlRoleStorageClient> _logger = logger;
    ...
    public async Task<Common.Role.Role?> AddAsync(Common.Role.Role? role, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Adding role.");
        var sql = await RoleDatabaseCommandText.GetInsertSql(role).ConfigureAwait(false);
        var result = await _dbClient.QueryAsync<Common.Role.Role>(sql, cancellationToken).ConfigureAwait(false);
        var inserted = result.FirstOrDefault();
        _logger.LogDebug("StorageClient: Added role.");
        return inserted;
    }
