...
namespace PharmacyManagementSystem.Server.Data.PostgreSql.Prescription;

public class NpgsqlPrescriptionStorageClient(ILogger<NpgsqlPrescriptionStorageClient> logger, INpgsqlDbClient dbClient) : IPrescriptionStorageClient
{
    private readonly ILogger<NpgsqlPrescriptionStorageClient> _logger = logger;
    ...
    public async Task<Common.Prescription.Prescription?> AddAsync(Common.Prescription.Prescription? prescription, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("StorageClient: Adding prescription.");
        var sql = await PrescriptionDatabaseCommandText.GetInsertSql(prescription).ConfigureAwait(false);
        var result = await _dbClient.QueryAsync<Common.Prescription.Prescription>(sql, cancellationToken).ConfigureAwait(false);
        var inserted = result.FirstOrDefault();
        _logger.LogDebug("StorageClient: Added prescription.");
        return inserted;
    }
